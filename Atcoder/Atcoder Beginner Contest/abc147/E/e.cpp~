	...
	FOR(i,1,h)
		FOR(j,1,w)
			FOR(k,0,6400)
				if(dp[i][j][k]){
					if(i+1<=h){
						int z=k+abs(a[i+1][j]-b[i+1][j]);
						if(z<6400) dp[i+1][j][z]=true;
						z=abs(k-abs(a[i+1][j]-b[i+1][j]));
						if(z<6400) dp[i+1][j][z]=true;
					}
					if(j+1<=w){
						int z=k+abs(a[i][j+1]-b[i][j+1]);
						if(z<6400) dp[i][j+1][z]=true;
						z=abs(k-abs(a[i][j+1]-b[i][j+1]));
						if(z<6400) dp[i][j+1][z]=true;
					}
				}
	FOR(k,0,6400)

...
const int N=3e5+5;
int n;
int a[N],cnt[N],d[N],pf[N],sf[N],f[N];
...
int main() {
	...
	REP(i,N) ++d[cnt[i]];
	pf[0]=0;
	FOR(i,1,n) pf[i]=pf[i-1]+i*d[i];
	sf[n]=d[n];
	FORD(i,n-1,0) sf[i]=sf[i+1]+d[i];
	FOR(i,1,n) f[i]=(pf[i]+sf[i+1]*i)/i;
	...
	res[n-1]=v[n-1].se;
	FORD(i,n-2,0)
		res[i]=max(res[i+1],v[i].se);
	int i=0;
	FOR(k,1,n){
		while(i<n&&v[i].fi<k) ++i;
		if(i==n){
			cout<<0<<'\n';
			continue;
		}

...
ll nCk(int n,int k){
	ll res=f[n];
	res=res*fpow(f[n-k],MOD-2)%MOD;
	res=res*fpow(f[k],MOD-2)%MOD;
	return res;
}

int main() {
	ios_base::sync_with_stdio(0);cin.tie(0);
	// consider x-axis and y-axis separately
	// consider each row in x-asix separately...
	f[0]=1;
	FOR(i,1,MX-1)
		f[i]=f[i-1]*i%MOD;
	cin>>n>>m>>k;
	FOR(i,1,n){
		ll t=(i-1)*m%MOD;
		ll g=(n-i)*m%MOD;
		res+=(t-g+MOD)%MOD*i%MOD*m%MOD;
		res%=MOD;
	}		
	FOR(i,1,m){
		ll t=(i-1)*n%MOD;
		ll g=(m-i)*n%MOD;
		res+=(t-g+MOD)%MOD*i%MOD*n%MOD;
		res%=MOD;
	}
	res=res*nCk(n*m-2,k-2)%MOD;
	cout<<res<<'\n';
}

...
int main() {
	...
	sort(all(v),[](const pair<string,pair<int,int>>& L,const pair<string,pair<int,int>>& R){
				if(L.fi==R.fi) return L.se.fi>R.se.fi;
				else{
					for(int i=0;i<min(sz(L.fi),sz(R.fi));++i)
						if(L.fi[i]<R.fi[i]) return true;
						else if(L.fi[i]>R.fi[i]) return false;
					return sz(L.fi)<sz(R.fi);	
				}
			});
	for(auto i:v)
		cout<<i.se.se+1<<'\n';

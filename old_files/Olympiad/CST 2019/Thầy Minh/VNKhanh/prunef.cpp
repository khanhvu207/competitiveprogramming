...
void trace(int u, int d)
{
    if ((int)g[u].size())
    {
        ...
        if ((int)g[u].size()==1)
        {
            tmp=min(tmp, solve(g[u][0], d+(w[u]?-1:1)));
            if (tmp>1 && d==0)
            {
                res.push_back(u);
            } else trace(g[u][0], d+(w[u]?-1:1));
        }
        else
        {
            int p=-1;
            for (int i=-n; i<=n; ++i)
            {
                if (solve(g[u][0], i)+solve(g[u][1], d+(w[u]?-1:1)-i)<tmp)
                    tmp=solve(g[u][0], i)+solve(g[u][1], d+(w[u]?-1:1)-i), p=i;
            }
            if (tmp>1 && d==0)
            {
                res.push_back(u);
